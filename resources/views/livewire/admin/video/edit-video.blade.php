{{-- resources/views/livewire/admin/video/edit-video.blade.php --}}
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-3xl text-white font-bold">Edit Video Pembelajaran</h1>
        </div>
        <p class="text-gray-400">Perbarui informasi video {{ $video->judul_video }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg">
        <form wire:submit.prevent="save">
            <div class="p-6 space-y-6">
                ...
                {{-- Section: File Saat Ini --}}
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                        <i class="fas fa-file text-pink-500 mr-2"></i>
                        File Saat Ini
                    </h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <i class="fas fa-file-video text-2xl text-pink-500"></i>
                            </div>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">
                                {{ basename($video->video_path) }}
                            </p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                Ukuran: {{ $this->fileSize }}
                            </p>
                        </div>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Subtitle File</span>
                                @if($video->subtitle_path && Storage::disk('public')->exists($video->subtitle_path))
                                    <i class="fas fa-closed-captioning text-2xl text-green-500"></i>
                                @else
                            ...
                            @if($video->subtitle_path && Storage::disk('public')->exists($video->subtitle_path))
                                <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">
                                    {{ basename($video->subtitle_path) }}
                                </p>
                                <label class="inline-flex items-center cursor-pointer">
                                    <input 
                                        type="checkbox" 
                                        wire:model.live="remove_subtitle"
                                        class="rounded border-gray-300 text-red-600 shadow-sm focus:border-red-300 focus:ring focus:ring-red-200 focus:ring-opacity-50"
                                    >
                                    <span class="ml-2 text-xs text-red-600 dark:text-red-400">Hapus subtitle</span>
                                </label>
                            @else
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    Tidak ada subtitle
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <p class="text-sm text-blue-800 dark:text-blue-200">
                                    Kosongkan jika tidak ingin mengubah file. File lama akan otomatis dihapus jika Anda upload file baru.
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-3">
                                <label class="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer transition-colors">
                                    ...
                                    <input 
                                        type="file" 
                                        wire:model="new_video_file"
                                        accept="video/*"
                                        class="hidden"
                                    >
                                </label>
                                
                                <div wire:loading wire:target="new_video_file" class="flex items-center text-pink-600">
                                        ...
                                        <div class="flex items-center min-w-0 flex-1">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                ...
                                                <p class="text-xs text-green-600 dark:text-green-400 mt-0.5">
                                                    {{ number_format($new_video_file->getSize() / 1024 / 1024, 2) }} MB
                                                </p>
                                            </div>
                                        </div>
                                        <button 
                                            type="button"
                                            wire:click="$set('new_video_file', null)"
                            ...
                            @error('new_video_file')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>

                        {{-- Upload Subtitle Baru --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                File Subtitle Baru
                            </label>
                            ...
                            <div class="flex items-center gap-3">
                                <label class="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer transition-colors">
                                    <i class="fas fa-upload mr-2"></i>
                                    <span>Pilih Subtitle Baru</span>
                                    <input 
                                        type="file" 
                                        wire:model="new_subtitle_file"
                                        accept=".srt,.vtt,.txt"
                                        class="hidden"
                                    >
                                </label>
                                
                                <div wire:loading wire:target="new_subtitle_file" class="flex items-center text-green-600">
                                    <svg class="animate-spin h-4 w-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                ...
                                <div class="mt-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center min-w-0 flex-1">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <p class="text-sm font-medium text-green-800 dark:text-green-200 truncate">
                                                    {{ $new_subtitle_file->getClientOriginalName() }}
                                                </p>
                                                <p class="text-xs text-green-600 dark:text-green-400 mt-0.5">
                                                    {{ number_format($new_subtitle_file->getSize() / 1024, 2) }} KB
                                                </p>
                                            </div>
                                        </div>
                                        <button 
                                            type="button"
                                            wire:click="$set('new_subtitle_file', null)"
                                            class="ml-3 text-red-600 hover:text-red-800 dark:text-red-400"
                                        >
                                            <i class="fas fa-times"></i>
                                        </button>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 flex items-center justify-end space-x-3 rounded-b-lg">
                ...
                <button 
                    ...
                    wire:target="save, new_video_file, new_subtitle_file"
